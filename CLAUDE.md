...
11. **코드 수정 시 해당 파일의 ESLint 에러를 반드시 함께 수정한다.** 파일을 수정·생성한 후 커밋 전에 `npx next lint --file {수정한파일}` 을 실행하여 에러가 없는지 확인한다. 기존 코드에 이미 존재하던 ESLint 에러라도, 해당 파일을 건드린 이상 함께 수정해야 한다. ESLint 에러가 남아있으면 dev 서버가 500을 뿜는다. (실제 사고 발생 이력 있음 — `no-unused-expressions` 삼항 표현식이 컴파일 실패 유발)
