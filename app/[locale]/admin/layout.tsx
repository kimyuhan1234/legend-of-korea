...
export default function AdminLayout({ children, params }: AdminLayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex">
      ...
      {/* Sidebar */}
      <aside className={`
        fixed inset-y-0 left-0 z-50 w-72 bg-[#1B2A4A] text-white transition-transform duration-300 ease-in-out transform
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="p-6 flex items-center justify-between">
            <Link href={`/${locale}`} className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-lg bg-[#D4A843] flex items-center justify-center font-black text-[#1B2A4A]">
                L
              </div>
              ...
              return (
                <Link
                  ...
                  className={`
                    ...
                    ${active 
                      ? 'bg-[#D4A843] text-[#1B2A4A] font-bold shadow-lg shadow-[#D4A843]/20' 
                      : 'hover:bg-white/5 text-slate-400 hover:text-white'}
                  `}
                >
                  <item.icon className={`w-5 h-5 ${active ? 'text-[#1B2A4A]' : 'group-hover:scale-110 transition-transform'}`} />
        ...
        <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-6 sticky top-0 z-30">
          ...
          <div className="flex items-center gap-4 ml-auto">
            <div className="text-right hidden sm:block">
              <p className="text-sm font-bold text-[#1B2A4A]">관리자</p>
              <p className="text-[10px] text-slate-500 uppercase tracking-widest font-black">Super Admin</p>
            </div>
            <div className="w-10 h-10 rounded-full bg-[#1B2A4A] text-[#D4A843] flex items-center justify-center font-black ring-2 ring-slate-100">
              A
            </div>
          </div>
        </header>
