...
export function OpenMission({ 
  ...
  return (
    <Card className={`w-full overflow-hidden border-2 rounded-[2.5rem] shadow-xl backdrop-blur-md transition-all duration-500 ${themeColors}`}>
      <CardHeader className={`${isBoss ? 'bg-peach/30' : isHidden ? 'bg-lavender/30' : 'bg-primary/5'} pb-6 pt-8 px-8`}>
        <div className="flex justify-between items-center mb-4">
          <Badge variant="outline" className={`flex items-center gap-1.5 py-1 px-3 ${
              isBoss ? 'bg-peach text-slate border-blossom' : isHidden ? 'bg-lavender text-sky border-lavender' : 'border-primary/30 text-primary'
          }`}>
          ...
          <div className="space-y-8">
            ...
            <Button 
                ...
                className={`w-full h-16 rounded-[1.5rem] shadow-2xl transition-all font-black text-xl flex items-center gap-2 ${
                    isBoss ? 'bg-peach0 hover:bg-blossom' : isHidden ? 'bg-sky hover:bg-sky/80' : 'bg-primary hover:bg-primary/90'
                }`}
            >
                {isSubmitting ? (
                    <>
