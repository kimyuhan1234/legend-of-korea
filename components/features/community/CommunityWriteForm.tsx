        ...
        <CardContent className="p-8 space-y-8">
          ...
          <div className="space-y-3">
            ...
            <div className="min-h-[56px] rounded-2xl border border-slate-100 bg-slate-50/50 p-3 flex flex-wrap gap-2 items-center focus-within:ring-2 focus-within:ring-indigo-100 transition-all">
              {tags.map((tag, i) => (
                <span key={i} className="flex items-center gap-1.5 px-3 py-1.5 bg-indigo-50 text-indigo-600 rounded-xl text-sm font-bold">
                  {tag.startsWith('#') ? tag : `#${tag}`}
                  <button onClick={() => removeTag(i)} className="hover:bg-indigo-200/50 rounded-full p-0.5 transition-colors">
                    <X className="w-3 h-3" />
                  </button>
                </span>
              ))}
          ...
          <div className="flex items-center gap-3 p-4 bg-indigo-50/30 rounded-2xl border border-indigo-100/30">
            ...
            <p className="text-xs font-bold text-indigo-600/80">
              이야기를 기록하면 <span className="text-indigo-600 font-extrabold">{images.length > 0 ? '50 LP' : '30 LP'}</span>가 즉시 적립됩니다.
            </p>
          </div>
