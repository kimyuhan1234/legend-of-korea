        ...
        return (
          <div
            ...
            className={`relative bg-white rounded-3xl p-6 border-2 transition-all ${
              isLegend
                ? 'border-mint-deep shadow-[0_12px_40px_rgba(255,107,53,0.15)] scale-[1.02]'
                : 'border-mist/60 hover:border-mint-deep/40'
            }`}
          >
            {isLegend && (
              <span className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 rounded-full bg-gradient-to-br from-mint to-blossom text-ink text-[10px] font-black uppercase tracking-widest shadow-md">
                {t('subscription.best')}
              </span>
            )}

            <div className="text-center mb-5">
              <p className="text-sm font-bold text-mint-deep uppercase tracking-widest mb-2">
                {getName(plan)}
              </p>
              <p className="text-3xl font-black text-[#111]">
                ₩{plan.price.toLocaleString()}
                <span className="text-xs font-normal text-stone ml-1">
            ...
            {plan.kit_discount_rate > 0 && (
              <div className="mb-3 px-3 py-2 rounded-xl bg-snow border border-mint-deep/20">
                <p className="text-[10px] font-black text-mint-deep text-center">
            ...
            {plan.tier_levelup && (
              <div className="mb-3 px-3 py-2 rounded-xl bg-gradient-to-r from-mint-light/30 to-mint/20 border border-mint-deep/20">
                <p className="text-[10px] font-black text-mint-deep text-center">
