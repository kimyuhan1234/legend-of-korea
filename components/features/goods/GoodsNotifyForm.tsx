...
export function GoodsNotifyForm({
  ...
  if (status === "success") {
    return (
      <div className="flex items-center gap-3 px-5 py-4 rounded-2xl bg-[#F0B8B8]/10 border border-blossom-deep/30">
        ...
        <input
          ...
          className={`flex-1 rounded-xl border-2 px-4 py-3 text-sm outline-none transition-colors ${
            isInputError
              ...
              : "border-mist focus:border-blossom-deep"
          }`}
        />
        <button
          type="submit"
          disabled={status === "loading"}
          className="px-6 py-3 rounded-xl bg-[#F0B8B8] hover:bg-blossom text-[#111] font-bold text-sm transition-colors whitespace-nowrap disabled:opacity-60 disabled:cursor-not-allowed"
        >
